...
VideoH264Encoder::VideoH264Encoder(const VideoCodecParameters& par)
 ...
 *  类型参数换成普通变量类型
 */
void VideoH264Encoder::setParameters(VideoCodecParameters pram)
{
...
bool VideoH264Encoder::checkParametersValidate(VideoCodecParameters parameters)
{
    VideoCodecParameters par = parameters;
    bool ok = true;
    if (par.getBitrate() <= 0) {
        ok = false;
